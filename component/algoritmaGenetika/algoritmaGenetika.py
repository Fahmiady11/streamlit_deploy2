...
def crossover(parent,pr):
    ...
    while(len(child)<total_child):
      ...
      if(rand1!=rand2):
        ...
        hasilSilang1 = flattened_list1[:pointMin] + slicePop2 + flattened_list1[pointMax+1:]
        hasilSilang2 = flattened_list2[:pointMin] + slicePop1 + flattened_list2[pointMax+1:]
      ...
      for j in range(total_rand):
        ...
        while(cond):
          ...
          if(randMut not in temp):
            ...
            mutation=abs(round(min(random.randint(900, 1000),genRand+(randVal*(1000-1)))))
            parentRand =  parentRand[:randMut]+[mutation]+parentRand[randMut+1:]
            cond=False
      temp=[]
  ...
  for k in range(len(temp)):  #10
    for l in range(len(temp[k])): #kromosom 4
      for m in range(len(temp[k][l])): #gen
        konversi=round(((temp[k][l][m]/1000)*(sem[l][1]-sem[l][0]))+sem[l][0])
        temp[k][l][m] = konversi
